...
from patient.models import Prescription, Medicines, Lab_examination
...
def doctor_add_lab_test(request):
    if request.method == 'POST':
        ...
        ps=Prescription.objects.get(id=prescription_id)
        Lab_examination.objects.create(prescription=ps,test_name=test_name,test_type=test_type,instruction=instruction,
                                 ).save()
        return redirect('doctor_add_prescription',appointment_id)
